...
from src.module.local_search import two_opt_random_subset
from src.module.local_search import untangle_with_partial_two_opt
...
class TestLocalSearch(unittest.TestCase):
    ...
    def test_two_opt2(self):
        ...
        node_cords = {1: (0, 0), 2: (3, 0), 3: (3, 4), 4: (0, 4), 5: (1, 1)}
    ...
    def test_partial_two_opt2(self):
        ...
        # Initial chromosome (with crossing edges)
        chromosome = [1, 3, 2, 4, 5, 1]

        # Untangle crossing edges and optimize with partial 2-opt
        result_chromosome = untangle_with_partial_two_opt(
            chromosome, node_cords, dist_matrix, max_iterations=100
        )

        # Ensure the resulting chromosome has the same number of genes
        self.assertEqual(len(chromosome), len(result_chromosome))

        # Calculate fitness before and after optimization
        initial_fitness = fitness(chromosome, dist_matrix)
        result_fitness = fitness(result_chromosome, dist_matrix)
    ...
    def test_two_opt_random_subset(self):
        # Sample NODE_COORD_SECTION dictionary
        node_cords = {
            1: (565.0, 575.0),
            2: (25.0, 185.0),
        ...
        # Sample chromosome (city IDs in a random order)
        chromosome = [7, 8, 3, 4, 5, 6, 1, 2]

        # Perform the 2-opt local search
        improved_chromosome = two_opt_random_subset(chromosome, dist_matrix)
        ...
        self.assertLessEqual(initial_fitness, improved_fitness)
